    ...
    def publish(self):
        ...
        for topic in self._topics:
            if not topic.get_resources():
                raise ValueError(
                    f"Topic '{topic.title.value}' must have at least one resource"
                )
...
class Enrollment:
    ...
    def __init__(
        self, enrollment_id: EnrollmentId, student_id: UserId, course_id: CourseId
    ):
    ...
    def submit_assignment(
        self, assignment_id: AssignmentId, content: str
    ) -> Submission:
